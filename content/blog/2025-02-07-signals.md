...
<style>
/* Dark mode for the wavedrom diagrams. Wavedrom draws every diagram as an
   <svg class="WaveDrom"> whose shapes are <use> references to symbols from
   its "default" skin. Browsers do not match ancestor selectors (such as
   [data-theme="dark"]) inside <use> shadow trees, but inherited custom
   properties do reach them. Hence the colours are defined as custom
   properties on .WaveDrom and applied through the skin's own classes. The
   skin classes are qualified with "path" because the syntax highlighter
   uses .s1 and .s2 too. */
.WaveDrom {
  --wd-ink: #000;
  --wd-surface: #fff;
  --wd-blue: #b9e0ff;   /* wave "5": the next action in the interactive simulator */
  --wd-cyan: #ccfdfe;   /* wave "6": the current value of the consumed signal */
  --wd-accent: #0041c4; /* signal names, edges and edge labels */
}
[data-theme="dark"] .WaveDrom {
  --wd-ink: #e0e0e0;
  --wd-surface: #161b22;
  --wd-blue: #2b5b9a;
  --wd-cyan: #1f5f66;
...
.WaveDrom rect[style*="fill:#FFF"] {
  fill: var(--wd-surface) !important;
}
/* Edges and their end markers. Marker contents behave like <use> shadow
   trees, so these selectors must not rely on ancestors either. */
.WaveDrom path[style*="stroke:#00F"],
path[style*="stroke:#0041c4"] {
  stroke: var(--wd-accent) !important;
}
marker[style*="fill:#0041c4"] {
  fill: var(--wd-accent) !important;
}
path.s1, path.s2, path.s3, path.s4 {
  stroke: var(--wd-ink) !important;
}
path.s6 {
...
script.src = "script.js?q=" + Date.now();
