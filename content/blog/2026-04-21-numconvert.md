...
In simulation, this will always work as expected. `Integer`s scale to any size dynamically. In the HDL produced by clash-the-compiler, `Integer` becomes a 64-bit value<sup id="footnote-1-back"><a href="#footnote-1">1</a></sup>. This makes the following code:
...
# Footnotes
<sup id="footnote-1"><a href="#footnote-1-back">1</a></sup>Clash translates `Natural` and `Integer` to 64-bit values in HDL. This is a historical artifact that will likely be removed in future versions of Clash.
